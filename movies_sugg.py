...
def get_sorted_recommendations(movie_lst):
    ...
    movies_posters = [get_movie_poster(movie)
...
def get_related_titles(movie_lst):
    ...
    multiple_movies = {
        y for movie in movie_lst for y in get_movies_from_tastedive(movie)}
...
def make_poster(posters):
    ...
    all_pictures = [Image.open(poster).convert('RGB')
